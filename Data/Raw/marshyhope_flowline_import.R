...
# Find bounding box of stations ----
##  Used in getflowlines() to only download a certain area
lat_long <- fread('data/derived/sturgeon_detections.gz')

lats <- range(lat_long$lat)
longs <- range(lat_long$long)
...
flowlines <- flowlines[grepl('Marshy', flowlines$gnis_name),]
